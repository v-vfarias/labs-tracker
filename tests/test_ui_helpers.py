...
from labs_tracker.ui import formatting
...
class UIHelperTests(unittest.TestCase):
    def test_unknown_classification_includes_missing_and_unrecognized_values(self):
        ...
        query = {"typeOfIssue": tracking._issue_type_query("Unknown")}
        self.assertEqual([row["id"] for row in collection.find(query)], ["missing", "unknown", "invalid"])
        query = {"typeOfIssue": tracking._issue_type_query("SDK/code issues")}
    ...
    def test_resolution_aliases_and_missing_classification(self):
        self.assertIn("Updated code/sample", tracking._resolution_query("Fixed in lab")["$in"])
        self.assertNotIn("Unknown", tracking._resolution_query("Unknown")["$nin"])
        self.assertEqual(tracking._needs_classification_query(), {"$or": [
            {"typeOfIssue": tracking._issue_type_query("Unknown")},
            {"resolution": tracking._resolution_query("Unknown")},
        ]})
    ...
    def test_product_display_defaults_and_alias_deduplication(self):
        ...
        self.assertEqual(repo_products(repo_id, []), ["Foundry"])
        self.assertEqual(repo_products(repo_id, ["Azure AI Language", "Foundry", "PowerBI"]), ["Foundry", "Power BI"])
        self.assertEqual(repo_products(None, None), [])

    def test_datetime_input_and_display_contracts(self):
        self.assertIsNone(tracking._parse_dt("  "))
        self.assertEqual(tracking._parse_dt("2026-09-18T12:00:00+02:00"), datetime(2026, 9, 18, 10, tzinfo=timezone.utc))
        self.assertEqual(tracking._parse_dt("2026-09-18"), datetime(2026, 9, 18, tzinfo=timezone.utc))
        self.assertEqual(formatting._fmt_dt(None), "")
        self.assertEqual(formatting._fmt_table_dt("2026-09-18T12:00:00Z"), "2026-09-18 12:00")
        with self.assertRaises(ValueError):
            tracking._parse_dt("not a date")

    def test_csv_and_table_event_shapes(self):
        self.assertEqual(tracking._parse_csv(" Foundry, ,GitHub "), ["Foundry", "GitHub"])
        self.assertEqual(tracking._parse_csv([" Foundry ", None, ""]), ["Foundry"])
        self.assertEqual(tracking._parse_csv(None), [])
        row = {"issueId": "owner/repo#3"}
        for args in ({"row": row}, [None, row], (None, row)):
            self.assertEqual(formatting._table_event_row(args), row)
        for args in (None, [], {"row": "invalid"}, row):
            self.assertEqual(formatting._table_event_row(args), {})

    def test_links_and_chart_limits(self):
        self.assertEqual(formatting._issue_url("owner/repo#3", "PR"), "https://github.com/owner/repo/pull/3")
